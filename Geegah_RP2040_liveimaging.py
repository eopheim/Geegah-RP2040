...
for jj in range(frames):
    ...
    MAG_AIR  = np.sqrt(np.square(I_AE_VOLTS )+np.square(Q_AE_VOLTS))
    PHASE_AIR = np.arctan2(I_AE_VOLTS, Q_AE_VOLTS)
    time.sleep(0.001)
...
for jj in range(numFrames):
    ...
    #write DAT FILES
    sample_file_name = rawdata_save_dir+"frame"+str(jj)+".dat"
    geegah_hp.writeFile(sample_file_name, frames_data_nD_sample)
    
    if liveplot == True:
        _,_,I_SE_VOLTS, Q_SE_VOLTS = geegah_hp.loadRawByteDataRP2040(frames_data_nD_sample)
        MAG_SAMPLE = np.sqrt(np.square(I_SE_VOLTS )+np.square(Q_SE_VOLTS ))
        PHASE_SAMPLE = np.arctan2(I_SE_VOLTS , Q_SE_VOLTS )
        MAG = MAG_SAMPLE - MAG_AIR
        PHASE = PHASE_SAMPLE - PHASE_SAMPLE
        #plot MAG
        mytitle.set_text(base_title+str(jj)+' of ' +str(numFrames-1))
...
I_A_E, Q_A_E= np.array(I_A_E),np.array(Q_A_E)
...
MAG_AIR = np.sqrt(np.square(I_A_E_av)+np.square(Q_A_E_av ))
PHASE_AIR = np.arctan2(I_A_E_av, Q_A_E_av)

MAG_SAMPLE = []
PHASE_SAMPLE = []

for frame in range(num_SAMPLE_Frames):
    MAG = np.sqrt(np.square(I_S_E[frame])+np.square(Q_S_E[frame]))
    PHASE = np.arctan2(I_S_E[frame], Q_S_E[frame])
